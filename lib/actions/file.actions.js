...
export async function uploadFile({ file, accountId, path, uploadedBy }) {
    ...
    try {
        ...
        // Grant global read access (Role.any()) for the preview link to work in the browser.
        // Grant the owner (accountId) update and delete access for full control.
        const permissions = [
...
export async function updateFileAccess({ fileId, emails, path }) {
    ...
    try {
        ...
        if (existing.bucketField) {
            try {
                // We use Role.any() for read so the link works without session conflicts.
                // Obfuscation (the secret ID) provides the privacy layer.
                const storagePermissions = [
                    Permission.read(Role.any()),
                    Permission.update(Role.user(existing.accountId)),
                    Permission.delete(Role.user(existing.accountId)),
                ];
